...
export default function NewProduct() {
  ...
  // Update form default value for location when userData is available
  // This useEffect is no longer needed as defaultValues handles initial setting
  // and the field is now editable.
  // useEffect(() => {
  //   if (userData?.farmLocation) {
  //     form.setValue("location", userData.farmLocation);
  //   }
  // }, [userData, form]);
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      console.log("Filtered product data before Firestore:", filteredProductData, "Image URLs:", filteredProductData.imageUrls);
      await addDoc(collection(db, "products"), filteredProductData);

      // 3. Increment product count for the category
      const categoryRef = doc(db, "categories", values.category);
      ...
      toast({
        title: "Product Added!",
        description: "Your product has been successfully added and submitted for approval.",
      });
  ...
  useEffect(() => {
    if (!loading && (!user || !userData || !userData.isVerified)) {
      toast({
        ...
        description: "Please complete your farm verification to add new products.",
        variant: "destructive",
      });
  ...
  if (loading || dataLoading || !user || !userData || !userData.isVerified) {
    return (
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>
                        Enter the location where the product is available. Defaults to your farm&apos;s registered location.
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
